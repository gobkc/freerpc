import json
import re
import uuid
from dataclasses import dataclass, field
from typing import Any, List, Optional

import gi

gi.require_version("Gtk", "4.0")
from gi.repository import Gdk, Gtk, Pango


@dataclass
class JsonNode:
    id: str
    key: str
    value: Any
    parent: Optional["JsonNode"] = None
    children: List["JsonNode"] = field(default_factory=list)
    expanded: bool = True

    def is_leaf(self):
        return not isinstance(self.value, (dict, list))


class EditableJsonTree(Gtk.TextView):
    THEMES = {
        "light": {
            "background": "white",
            "key": "#9c27b0",
            "string": "#e91e63",
            "number": "#2e7d32",
            "boolean": "#f57c00",
            "null": "#757575",
            "icon": "#1976d2",
            "bracket": "#1e88e5",
            "comma": "#616161",
        },
        "dark": {
            "background": "black",
            "key": "#ce93d8",
            "string": "#f48fb1",
            "number": "#81c784",
            "boolean": "#ffb74d",
            "null": "#bdbdbd",
            "icon": "#64b5f6",
            "bracket": "#4fc3f7",
            "comma": "#9e9e9e",
        },
    }

    TAGS = ["key", "string", "number", "boolean", "null", "icon", "bracket", "comma"]

    def __init__(self, data=None):
        super().__init__()

        self.set_monospace(True)
        self.set_editable(True)
        self.set_wrap_mode(Gtk.WrapMode.NONE)
        self.set_vexpand(True)
        self.set_hexpand(True)

        self._current_theme = "light"

        self.buffer = self.get_buffer()
        self.buffer.set_enable_undo(True)

        self.node_map = {}
        self.icon_ranges = []

        self.root = self._build(data or {})

        self._create_tags()
        self._render()

        self._init_events()

    # --------------------------
    # 初始化事件
    # --------------------------
    def _init_events(self):
        click = Gtk.GestureClick()
        click.set_button(0)
        click.set_propagation_phase(Gtk.PropagationPhase.CAPTURE)
        click.connect("pressed", self._on_click)
        self.add_controller(click)

        key = Gtk.EventControllerKey()
        key.connect("key-pressed", self._on_key_pressed)
        self.add_controller(key)

        drag = Gtk.DragSource()
        drag.set_actions(Gdk.DragAction(0))
        self.add_controller(drag)

    # --------------------------
    # 主题
    # --------------------------
    def set_theme(self, theme_name: str):
        if theme_name not in self.THEMES:
            raise ValueError("theme must be light/dark")

        if self._current_theme == theme_name:
            return

        self._current_theme = theme_name
        cursor_offset = self._get_cursor_offset()

        self._create_tags()
        self._render()

        self._restore_cursor(cursor_offset)

    def _create_tags(self):
        table = self.buffer.get_tag_table()

        # 删除旧 tag
        for name in self.TAGS:
            tag = table.lookup(name)
            if tag:
                table.remove(tag)

        colors = self.THEMES[self._current_theme]

        def create(name, color, weight=None):
            tag = Gtk.TextTag.new(name)
            tag.set_property("foreground", color)
            if weight:
                tag.set_property("weight", weight)
            table.add(tag)

        create("key", colors["key"], Pango.Weight.BOLD)
        create("string", colors["string"])
        create("number", colors["number"])
        create("boolean", colors["boolean"])
        create("null", colors["null"])
        create("icon", colors["icon"])
        create("bracket", colors["bracket"])
        create("comma", colors["comma"])

    # --------------------------
    # 构建数据
    # --------------------------
    def _new_id(self):
        return str(uuid.uuid4())

    def _build(self, data, key="root", parent=None):
        node = JsonNode(self._new_id(), key, data, parent)
        self.node_map[node.id] = node

        if isinstance(data, dict):
            node.children = [self._build(v, k, node) for k, v in data.items()]
        elif isinstance(data, list):
            node.children = [self._build(v, "", node) for v in data]

        return node

    # --------------------------
    # 渲染核心
    # --------------------------
    def _render(self):
        buf = self.buffer
        buf.begin_user_action()

        buf.set_text("")
        self.icon_ranges.clear()

        def insert(text, tag=None):
            start = buf.get_end_iter().get_offset()
            buf.insert(buf.get_end_iter(), text)
            end = buf.get_end_iter().get_offset()
            if tag:
                buf.apply_tag_by_name(
                    tag, buf.get_iter_at_offset(start), buf.get_iter_at_offset(end)
                )
            return start, end

        def walk(node, depth=0, parent_is_list=False):
            indent = "\t" * depth
            buf.insert(buf.get_end_iter(), indent)

            # icon
            if node.children:
                icon = "[-] " if node.expanded else "[+] "
                s, e = insert(icon, "icon")
                self.icon_ranges.append((s, e, node.id))

            # key
            if node.key != "root" and not parent_is_list:
                insert(f'"{node.key}": ', "key")

            # value
            if node.is_leaf():
                text, tag = self._format_value(node.value)
                insert(text, tag)
            else:
                insert("{" if isinstance(node.value, dict) else "[", "bracket")

            # children
            if node.children and node.expanded:
                insert("\n")
                for i, c in enumerate(node.children):
                    walk(c, depth + 1, isinstance(node.value, list))
                    if i < len(node.children) - 1:
                        insert(",", "comma")
                        insert("\n")
                insert("\n" + indent)

            # closing
            if not node.is_leaf():
                insert("}" if isinstance(node.value, dict) else "]", "bracket")

        walk(self.root)
        buf.end_user_action()

    # --------------------------
    # 工具函数
    # --------------------------
    def _format_value(self, v):
        if v is None:
            return "null", "null"
        if v is True or v is False:
            return str(v).lower(), "boolean"
        if isinstance(v, str):
            return f'"{v}"', "string"
        if isinstance(v, (int, float)):
            return str(v), "number"
        return str(v), None

    def _get_cursor_offset(self):
        it = self.buffer.get_iter_at_mark(self.buffer.get_insert())
        return it.get_offset()

    def _restore_cursor(self, offset):
        if offset < self.buffer.get_char_count():
            self.buffer.place_cursor(self.buffer.get_iter_at_offset(offset))

    # --------------------------
    # 交互
    # --------------------------
    def _on_click(self, gesture, n_press, x, y):
        ok, it = self.get_iter_at_location(int(x), int(y))
        if not ok:
            return

        offset = it.get_offset()

        for s, e, nid in self.icon_ranges:
            if s <= offset < e:
                gesture.set_state(Gtk.EventSequenceState.CLAIMED)

                node = self.node_map.get(nid)
                if node:
                    node.expanded = not node.expanded
                    off = self._get_cursor_offset()
                    self._render()
                    self._restore_cursor(off)
                return

    def _on_key_pressed(self, controller, keyval, keycode, state):
        ctrl = state & Gdk.ModifierType.CONTROL_MASK
        alt = state & Gdk.ModifierType.ALT_MASK

        if ctrl and keyval == Gdk.KEY_s:
            self._manual_render()
            return True

        if ctrl and alt and keyval == Gdk.KEY_l:
            self._manual_render()
            return True

        return False

    # --------------------------
    # 手动解析 JSON
    # --------------------------
    def _manual_render(self):
        text = self.buffer.get_text(*self.buffer.get_bounds(), False)
        cleaned = re.sub(r"\[-\]\s*|\[\+\]\s*", "", text)

        try:
            data = json.loads(cleaned)
        except json.JSONDecodeError:
            return

        self.node_map.clear()
        self.root = self._build(data)
        self._render()
        self.buffer.place_cursor(self.buffer.get_start_iter())
