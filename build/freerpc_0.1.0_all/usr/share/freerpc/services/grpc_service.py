...
def _require_binary(name: str, apt_package: str) -> str:
    path = shutil.which(name)
    if not path:
        raise RuntimeError(
            f"'{name}' not found on PATH. Install it with: sudo apt install {apt_package}"
        )
    return path
...
def dynamic_grpc_call(
    ...
    protoc_bin = _require_binary("protoc", "protobuf-compiler")
    grpc_plugin = _require_binary("grpc_python_plugin", "protobuf-compiler-grpc")
    ...
    if include_dir:
        protoc_args.insert(1, f"--proto_path={include_dir}")
    else:
        print(
    ...
    proc = subprocess.run(protoc_args, capture_output=True, text=True)
